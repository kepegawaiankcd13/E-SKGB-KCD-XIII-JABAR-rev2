        ...
        {activeTab === "tte" && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 max-w-7xl mx-auto">
            ...
            <div className="lg:col-span-4 space-y-4">
              ...
              {/* DATA RECOVERY FROM FIREBASE */}
              {onMigrateFromFirebase && (
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-3">
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${isMigrating ? "bg-amber-400 animate-pulse" : "bg-indigo-500"}`}></span>
                    <span className="text-xs font-bold uppercase tracking-wider text-slate-700">Data Recovery & Migration</span>
                  </div>
                  <p className="text-[11px] text-slate-600 leading-normal">
                    Pernah mengimpor data di sistem cloud lama? Anda dapat memulihkan seluruh data pegawai, KOP surat, rekam aktivitas, dan akun staff dari database Firebase lama ke database MongoDB Atlas yang baru ini secara otomatis.
                  </p>
                  <button
                    type="button"
                    onClick={onMigrateFromFirebase}
                    disabled={isMigrating}
                    className={`w-full py-2.5 text-[11px] uppercase tracking-wider font-extrabold rounded-xl text-white shadow-sm transition-all flex items-center justify-center gap-2 cursor-pointer ${
                      isMigrating 
                  ...
                  >
                    {isMigrating ? (
                      <>
                        ...
                        <span>Sedang Memproses Migrasi...</span>
                      </>
                    ) : (
